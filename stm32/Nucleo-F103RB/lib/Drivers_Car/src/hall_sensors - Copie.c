...
//New function Trenton
/**
 * @brief       Returns the current angle of the wheel for hall sensor considered.
 * @param       hall_identifier Number of the hall sensor to consider.
 * @retval      Current angle of the wheel if current sector is under number of sectors, ERROR_SENSOR_OUT_OF_RANGE if not.
*/
short HallSensor_compute_angle(Sensor_Enum hall_identifier){
	return (360/HALLSENSOR_NUMBER_OF_SECTORS)*HallSensor_sector[hall_identifier];
}
